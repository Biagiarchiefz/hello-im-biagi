  ...
  return (
    ...
    {project.id % 2 === 0 ? ( 
      ...
      <div className="flex flex-col gap-1 pt-5 items-end w-full">
        <h3 className="font-semibold text-[#7E62F3]">Project 0{project.id}</h3>
        ...
        <p className="max-w-[430px]">{project.technologies.map((tech) => ` [${tech}]`)}</p>
      ...
      <img src={project.image} alt="" className="max-w-[761px] absolute left-0" />
    </div>
  ) : (
          <div className="text-[#EDF0F7] flex mt-25 relative">
      ...
      <img src={project.image} alt="" className="max-w-[761px] absolute right-0" />
    </div>
  )} 
        </>
